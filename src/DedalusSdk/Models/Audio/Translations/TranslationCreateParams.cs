...
///
/// <para>Translates audio files in any supported language to English text using OpenAI's
/// Whisper model. Supports the same audio formats as transcription. Maximum file
/// size is 25 MB.</para>
///
/// <para>Args:     file: Audio file to translate (required)     model: Model ID
/// to use (e.g., "openai/whisper-1")     prompt: Optional text to guide the model's
...
/// </summary>
public record class TranslationCreateParams : ParamsBase
{
    ...
    public TranslationCreateParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public TranslationCreateParams(TranslationCreateParams translationCreateParams)
        : base(translationCreateParams)
    {
        this._rawBodyData = new(translationCreateParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static TranslationCreateParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, MultipartJsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(TranslationCreateParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    {
        return new UriBuilder(options.BaseUrl.ToString().TrimEnd('/') + "/v1/audio/translations")
        {
